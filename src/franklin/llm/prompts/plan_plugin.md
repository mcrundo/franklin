...
5. **Bias toward fewer, higher-quality reference files.** Aim for 10–15 references. Combining related concepts into one well-structured reference file is almost always the right call — 12 rich references beat 25 thin ones. *This bias applies to reference files only — it does NOT apply to commands, which should be harvested generously from actionable workflows, or to agents, which should be split by use case when the material supports both a reviewer and a planner.*

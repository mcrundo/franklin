...
def test_rate_limit_classified() -> None:
    exc = RateLimitError(message="rate limit", response=_mock_response(429), body=None)
...
def test_authentication_error_classified() -> None:
    exc = AuthenticationError(message="bad key", response=_mock_response(401), body=None)
    result = format_friendly_error(exc)
...
def test_permission_denied_classified() -> None:
    exc = PermissionDeniedError(message="denied", response=_mock_response(403), body=None)
    result = format_friendly_error(exc)
    assert "permission" in result.title.lower()


def test_529_overloaded_classified() -> None:
    exc = APIStatusError(message="overloaded", response=_mock_response(529), body=None)
    result = format_friendly_error(exc)
...
def test_500_generic_server_error_classified() -> None:
    exc = APIStatusError(message="kaboom", response=_mock_response(500), body=None)
    result = format_friendly_error(exc)
...
def test_400_client_error_not_retryable() -> None:
    exc = APIStatusError(message="bad shape", response=_mock_response(400), body=None)
    result = format_friendly_error(exc)
